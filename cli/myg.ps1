...
Actions:
  ...
  sheet id=<ID or URL> name=<SHEET> [range=A1:D100] [rows=N] [colors=false]  Get sheet data
  ...
  doc copytab id=<ID or URL> tab=<TAB_ID> name="NEW" [index=<N>]  Copy tab with styles
    Smart chips (in text or markdown):
      ...
      {{ DATE:YYYY-MM-DD:locale }}  Date chip with locale (default: en)
      {{ PERSON:email }}      Person mention chip
      {{ LINK:URL }}          Rich link chip
      {{ IMAGE:URL or FILE_ID }}  Inline image
